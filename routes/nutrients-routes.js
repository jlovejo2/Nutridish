...
router.get("/api/nutrients/:userEmail/:nutrients", function (req, res) {
  ...
  console.log(req.params.userEmail + "has searched for " + req.params.nutrients);
  ...
  db.Searches.findOne({
        ...
        axios.get(queryURL).then(function (apiRecipes) {
          ...
          db.Recipes.bulkCreate(createArr).then(function (createdRecipes) {
            ...
            newSearch.addRecipes(createdRecipes).then(function (resp) {
              ...
              const recipeData = resp.map(Recipe => Recipe.get({ plain: true }))

              const updatedCreateArr = createArr.map((indexCreateArr, indexRecipeData) => {
                indexCreateArr['id']=recipeData[indexRecipeData].RecipeId
              })

              // console.log(updatedCreateArr);
              
            //  const rawAddedRecipes = resp.dataValues.map(recipe => recipe.get({plain: true }))
            //  console.log(rawAddedRecipes);
              ...
              res.render('recipes', { 'layout': 'main.handlebars', recipe: createArr });
            }).catch(error => {
              console.error(error)
            })
    ...
    //If SQL query finds a search
      ...
      db.Searches.findOne({ where: { id: foundSearchId }, include: { model: db.Recipes } }).then(function (dbRecipes) {

        const recipeData = dbRecipes.dataValues.Recipes.map(Recipe => Recipe.get({ plain: true }))
